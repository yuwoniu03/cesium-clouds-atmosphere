...
| 依赖 | 说明 |
|---|---|
| **Cesium** | 作为**全局变量** `window.Cesium` 使用。本库**不**通过 npm `import` Cesium，使用方需自行引入 Cesium 并在全局挂载。 |
| **three** | 仅在 `loadBinThreeGeospatial.js` 中用于把 `.bin` 解析为 `Data3DTexture`（解析阶段不涉及 Cesium）。声明为 `peerDependencies`。 |
| **dat.gui** | 运行时调参面板。声明为 `dependencies`。 |
...
下面是与原项目 `mapCesium.vue` 调用方式等价的最小示例：
